...
- 排行榜：首页支持通过 KEY3 进入排行榜界面，按模式查看历史前 5 名分数。
- 持久化：最高分、音量、默认模式和分模式排行榜都会写入片内 Flash。
- 声光反馈：LED 指示当前移动方向，DAC 输出不同频率的音效和关卡背景旋律。
- 首页动效：首页与排行榜页中的装饰蛇图会在浅蓝背景块中循环向右移动。
- 显示优化：开放地图模式对视口刷新和掉命后的背景重绘做了针对性优化，减少残影和闪烁。
- 动态难度：蛇的移动间隔会随分数缩短，后期关卡进一步加快节奏。

需要说明的是，`Project/target.txt` 中曾提出 USART 键盘控制、Flash 最高分持久化等扩展方向。当前版本已经接入 USART1 键盘控制，并将最高分、音量、默认模式和分模式排行榜持久化写入片内 Flash。
...
- `KEY1(PD11)` / `KEY2(PD12)`：开始当前模式和当前关卡
- `KEY3(PC13)`：进入排行榜页
- `KEY4(PA0)`：进入设置页

排行榜页中，旋钮用于切换模式，任意物理方向键返回首页；串口端发送 `R` 也可返回首页。暂停状态下按 KEY1 或串口 `R` 同样可以返回首页。
...
克隆仓库后，以仓库根目录作为工作目录。`Project` 目录是 Keil/EIDE 工程目录，真正的业务源码主要在 `User` 目录中。
...
游戏初始生命值为 3。撞到边界、障碍物或自身时会损失 1 点生命；STAGE 模式下的限时关倒计时归零也会损失生命。只要生命值没有归零，系统会在当前关卡继续游戏。碰到边界、障碍物或超时后，系统会重新放置蛇和食物；当蛇头撞到自身时，则会扣除 1 点生命，并让被撞部位到蛇尾之间的整段蛇身消失，同时同步缩短长度并扣减相应分数。生命值为 0 时显示 `GAME OVER`，按 KEY1 或在串口端发送 `R` 后返回首页，重新选择模式和起始关卡。

在双人模式中，两名玩家各自维护蛇身和得分。任一玩家发生碰撞时，系统会根据死亡情况显示 `P1 WINS`、`P2 WINS` 或 `DRAW`，随后同样通过 KEY1 或串口 `R` 返回首页。

最高分、音量、默认模式和各模式排行榜都会写入片内 Flash，断电或复位后仍可保留。
...
VS Code + EIDE 配置已同步加入 `../User/Uart/snake_uart.c`、`../User/Uart` 头文件路径和标准外设库的 `stm32f10x_usart.c`。
...
- 如果 Keil 或 EIDE 报找不到头文件，检查包含路径是否包含 `Libraries/CMSIS/CoreSupport`、`Libraries/CMSIS/DeviceSupport`、`Libraries/STM32F10x_StdPeriph_Driver_v3.5/inc` 和 `User/Main`、`User/Lcd`、`User/Timer`、`User/Uart`。
